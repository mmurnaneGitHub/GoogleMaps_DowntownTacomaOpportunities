...
$(document).on('pageshow', '#pageone', function(){       
  ...
  //Calculate width & height of elements based on screen size
  resizeElements();

  //Resize page elemments when the device is turned
   $( window ).on( "orientationchange", function( event ) {
    resizeElements();
  });

  //Resize page elemments when page resized
  ...
  //Close menu , open specific POI tab ----------------------------------
  ...
  function resizeElements() {
   //Set initial widths - Start with map max size (otherwise blank tiles), then toggle POI panel open on initial load
   $('#POI').width(0); 
   $('#theMap').width('100%'); 

   //Calculate height of elements based on current screen size
   ...
   //Wait for map to load before sliding panel  - slide open on intial load, leave closed otherwise
   //see google.maps.event.addListenerOnce(gmap, 'idle', function()
  }
      ...
      google.maps.event.addDomListener(controlUI, 'click', function() {
          if (navigator.geolocation) {
              ...
              navigator.geolocation.getCurrentPosition(function (position) {
                ...
                //Examples for possible options: https://github.com/esri/html5-geolocation-tool-js
...
//SEE http://api.jquery.com/jquery.getjson/ for possible jQuery replacement
              ...
              for (var i=0; i<dataQuery.length; i++) {
                ...
                //DETAILED DESCRIPTIONS---------------------------------------
                  ...
                  var detail_content = "<div style='clear:both;width: 330px;'>";  //start address header - for mobile 
                       ...
                       detail_content += "<span style='color:rgb(56,64,142);'>" + dataQuery[i].Address + "</span>";
                        ...
                        detail_content += "&nbsp;|&nbsp;<a href=\"https://maps.google.com/maps?daddr=" + iAddress + ", Tacoma, WA\" target='_blank' onfocus='this.blur();' title='Get driving directions.'>Directions</a>";
  
                        //Zoom to location
                        detail_content += "&nbsp;|&nbsp;<a href='javascript:go2studio(" + dataQuery[i].Latitude + ", " + dataQuery[i].Longitude + ")' onfocus='this.blur();' title='Close this window and zoom map to location.'>Zoom</a>";

                        //Street View - links directly to Google Maps no longer work (view points north)
                        detail_content += "&nbsp;|&nbsp;<a href='javascript:toggleStreetView(" + dataQuery[i].Latitude + ", " + dataQuery[i].Longitude + ")' onfocus='this.blur();'  title='Zoom map to location and toggle Street-Indoor & Aerial View'>Street</a>";
                ...
                //end DETAILED DESCRIPTIONS-------------------------------------
                      ...
                      var part4 = "<div style='padding:2px; margin: 2px;'><b>" +  dataQuery[i].Name + "</b><br>" +  dataQuery[i].Description +  "</div>";
                          part4 += "<div style='clear:both;'><a title='Zoom map to location and toggle Street-Indoor & Aerial View' href='javascript:toggleStreetView(" + dataQuery[i].Latitude + ", " + dataQuery[i].Longitude + ")' >Street-Indoor & Aerial</a>";
  ...
  myText += "<img src='http://maps.google.com/maps/api/staticmap?center="; //need to leave as http (https won't appear open separately - in cache)
...
function toggleStreetView(newLat,newLong) {
   ...
   //If current map is small (portrait on a mobile device) then close POI panel
  if ($('#theMap').width()<300) {
     $('#POI').width(0); 
     $('#theMap').width('100%');
   } 
